...
characterises the physical abilities of robotic manipulators, at the same time being the most complete approximation of human physical abilities based on musculoskeletal models. 

Additionally, since most of the well known polytope formulations for robotic systems can be formulated as polytopes for musculoskeletal models as well, they enable evaluating and comparing human's and robot's physical abilities in a unified manner. Furthermore, the polytope algebra enables operations over multiple polytopes and, in that way, enables characterising joint physical abilities of humans and robots involved in different physical collaboration scenarios. 

% Having a set of accurate tools for evaluating physical abilities of humans, robots and their interactions could potentially 

% Being able to have an accurate information about robot's physical abilities enables creating robot's tasks capable of fully exporting robot's abilities, while making sure that they are not surpassed. This is particularly important with collaborative robots, which have limited performance due to their high safety requirements, and using more traditional global metrics, for example robot's payload, might not be practical.
% On the other hand, having accurate information about 
...
\paragraph*{Considering orientations}
It is worth noting that in general case, $m$ dimensional task space might include positions and orientations, for example if the task space corresponds to the Cartesian space, where $m=6$. When the task space includes orientations, the generalised task space position (including the position and orientation) is often expressed is the homogeneous matrix form $\bm{x}\in SE(3)$. Then the task space generalised velocity vector $\dot{\bm{x}}\in\mathbb{R}^6$ becomes the Cartesian space \textit{twist vector} $\bm{\nu}$, expressing the translation and rotation velocity at the same time. However, in such cases the twist $\bm{\nu}$ does no longer correspond to the time derivative of the task space generalised position $$\bm{\nu}\neq\frac{d\bm{x}}{dt}.$$ The correspondence between the generalised position, having a matrix form $\bm{x} \in \mathbb{R}^{4\times4}$, and the twist, having a vector form $\bm{\nu} \in \mathbb{R}^{6}$, is expressed through the matrix exponential and logarithm operations, as described in detail in \cite[Chapter 3.3]{modernrobotics}. 

Hence, a more generic mapping (\ref{eq:js_to_cs_vaj}) can be expressed as 
\begin{subequations}
\begin{align}
\bm{\nu}&= {J}(\bm{q})\dot{\bm{q}} \label{eq:js_to_cs_vaj:vel}\\
\dot{\bm{\nu}}&= J(\bm{q})\ddot{\bm{q}} + \dot{J}(\bm{q},\dot{\bm{q}})\dot{\bm{q}} \label{eq:js_to_cs_vaj:accel}\\
\ddot{\bm{\nu}}&= J(\bm{q})\dddot{\bm{q}} + 2\dot{J}(\bm{q},\dot{\bm{q}})\ddot{\bm{q}} + \ddot{J}(\bm{q},\dot{\bm{q}},\ddot{\bm{q}})\dot{\bm{q}}\label{eq:js_to_cs_vaj:jerk}
 \end{align} \label{eq:js_to_cs_vaj_twist}
\end{subequations}
where $\bm{\nu}\in\mathbb{R}^m$ corresponds to generalised task space velocities, while $\dot{\bm{\nu}}\in\mathbb{R}^m$ are $\ddot{\bm{\nu}}\in\mathbb{R}^m$ are generalised accelerations and jerks, allowing for considering both their translation and rotation components.

\todos{
In this work however, for simplicity reasons, the notation for task space variables representing the velocity as $\dot{\bm{x}}$, acceleration as $\ddot{\bm{x}}$ and jerk as $\dddot{\bm{x}}$ is used for both cases. 
}
...
Multi-link cable-driven robots (MCDRs) \cite{Wang2019} is a family of robotic manipulators which is actuated by cable actuators. Th


are robotic systems combining the serial and parallel robots. They have relatively large workspace compared to parallel robots and much lower inertia than the serial robots, resulting in higher achievable velocities and better back-drivability. 
...
Following two sections, \ref{ch:force_collab} and \ref{ch:velocity_collab}, give an example of calculation of joint force and velocity capacity of a physical human-robot collaboration based on polytope algebra. Both examples are based on a common physical collaboration scenario where two actors (usually human and robot) are interacting physically to manipulate an object that is rigidly fixed in the end-effector of the robot and in the hand of the human. Different versions of this scenario are shown on Figure \ref{fig:collaboration_types}, ranging from human-robot interaction to the interaction of the two arms of a human subject, that can be seen as two separate musculoskeletal models collaborating.

Finally, chapter \ref{ch:collab_metrics_overview} gives an overview of polytope based metrics for humans and robots and how to combine them to obtain the joint capacity of multiple actors in the discussed collaboration scenario. 
...
\subsection{Calculating different joint physical ability polytopes}
...
This section gives an overview of the common polytope based physical ability metrics for robotic systems, described in section \ref{ch:robot_metrics}, and for humans, described in section \ref{ch:human_metrics}, with respect to the polytope algebra operation necessary to calculate joint physical abilities in collaboration scenarios. 

For this comparison a human-robot physical interaction scenario from previous chapters, where the human operator and the robot are physically interacting in order to manipulate an object which is rigidly fixed both in the robot's end-effector, is chosen as a representative of a classic collaboration scenario. A simplified view of this collaboration scenario is shown on Figure \ref{fig:table_inter}, while a more general view of this scenario, with different actors (humans and robots), is shown on Figure \ref{fig:collaboration_types}.
...
As discussed in section \ref{ch:force_collab}, when it comes to commonly manipulating an object, as all the actors apply forces on the object the resulting force the object will exert on the environment (if in contact with it) or transform into the motion, will be the sum of the applied forces by the robot $\bm{f}_r$ and the human $\bm{f}_h$. 
\begin{equation}
    \bm{f} = \bm{f}_r + \bm{f}_h
\end{equation}
where each one of the forces is bounded in withing the polytope $\bm{f}_r \in \mathcal{P}_{fh}$. 
\begin{equation}
    \mathcal{P}_f = \{\bm{f}\in \mathbb{R}^m ~|~\bm{f} = \bm{f}_r + \bm{f}_h , \quad\bm{f}_r \in \mathcal{P}_{fr},~\bm{f}_h \in \mathcal{P}_{fh}\}
\end{equation}
Therefore their common wrench capacity can be expressed as a sum of their individual wrench capacities, corresponding to the Minkowski sum in polytope algebra $\oplus$.
\begin{equation}
...
On the other hand, as discussed in section \ref{ch:velocity_collab}, in the proposed collaboration scenario, the maximal velocity capacity of the object will be limited by the actor (human or robot) with the least velocity capacity. This is due to the fact that both robot and the human are rigidly attached to the object and the velocity of the object $\dot{\bm{x}}$ is equal to the velocity of the human and the robot $\dot{\bm{x}}_h=\dot{\bm{x}}_r=\dot{\bm{x}}$
\begin{equation}
...
\end{equation}
More generally, all polytope metrics characterising robot's or human's movement capacity (velocity $\dot{\bm{x}}$, acceleration $\ddot{\bm{x}}$, etc.) will be combined using the intersection $\cap$ operation.

The list of the common polytope based metrics, for humans and robots along with their discussed polytope algebra operator is shown in Table \ref{tab:table_comparisson_colloboration}.
...
This chapter brings a comprehensive overview of different physical ability metrics for humans and robots, with the aim to find an unifying view of their abilities and provide a framework for characterising their joint physical abilities. The focus of this chapter is therefore placed on polytope based representations of physical abilities as they are the most complete and the most accurate metrics for both humans and robots. Additionally, polytope algebra enables efficient operations over polytopes, such as sum and intersection, providing tools for characterising their common physical abilities when they are interacting physically to execute a task.


Characterising the physical abilities of humans (based on musculoskeletal models) and robots consists in describing the relationship between actuation limits and the achievable sets of task-related variables. As these relationships are usually given through robot's and human's highly nonlinear dynamics and kinematics relationships, in order to obtain the polytope-shaped sets, the dynamics and kinematics equations are linearised around a specific state $\{\bm{q},\dot{\bm{q}}\}$. Yielding linear and affine transformations of the actuator limits to the desired task space variables which can be represented with polytopes.

%The resulting polytope formulations involve a combination of equality equations (representing linearised dynamics and kinematics) and inequality equations (representing different actuation limits).

The section \ref{ch:poly_metrics} brings an overview of common polytope representations of physical abilities for robots and humans. 
Common polytope formulations, involving different linearised dynamics and kinematics equations and different actuator limits, can be represented using a generic formulation
\begin{equation}
    \mathcal{P}_x = \{\bm{x}\in\mathbb{R}^m ~|~ A\bm{x}=B\bm{y} + \bm{b}, ~ \bm{y}\in\mathcal{I}\}
\end{equation}
where the polytope $\mathcal{P}_x$ of represents the achievable set of the task space variable $\bm{x}\in\mathbb{R}^m$, $\bm{y}\in\mathbb{R}^n$ is the input variable limited within actuator limits $\mathcal{I}$. The linearised dynamics and kinematics equations can be represented using the equation $A\bm{x}=B\bm{y} + \bm{b}$, where 

\begin{table}[!h]
\centering
\begin{tabular}{|l|c|c|c|c|c|c|c|}
\hline
Polytope Metric & Eqn. & $\bm{x}$ & $A$ & $B$ & $\bm{y}$ & Input set $\mathcal{I}$ & $\bm{b}$ \\
\hline
 \multicolumn{8}{c}{Robotic manipulators} \\
\hline
Velocity  & \ref{eq:poly_vel_rob}& $\dot{\bm{x}}$ & - & $J$ & $\dot{\bm{q}}$& $[\dot{\bm{q}}_{min},\dot{\bm{q}}_{max}]$  & - \\
Kin. Acceleration  & \ref{eq:poly_accel_kin} & $\ddot{\bm{x}}$ & - & $J$ & $\ddot{\bm{q}}$ & $[\ddot{\bm{q}}_{min},\ddot{\bm{q}}_{max}]$   & $\bm{a}_b$ \\
Kin. Jerk  &\ref{eq:poly_jerk_kin} & $\dddot{\bm{x}}$ & - & $J$ & $\dddot{\bm{q}}$& $[\dddot{\bm{q}}_{min},\dddot{\bm{q}}_{max}]$   & $\bm{j}_b$ \\
Precision  & \ref{eq:poly_precision_rob} & $\delta\dot{\bm{x}}$ & - & $J$ & $\delta\bm{q}$ &$[\delta{\bm{q}}_{min},\delta{\bm{q}}_{max}]$ &  - \\
Force/Wrench &  \ref{eq:poly_force_rob} & $\bm{f}$ & $J^T$ & - & $\bm{\tau}$ &$[\bm{\tau}_{min},\bm{\tau}_{max}]$ &  -$\bm{\tau}_b$ \\
Acceleration  & \ref{eq:pol_accleration_rob} & $\ddot{\bm{x}}$ & - & $JM^{-1}$ & $\bm{\tau}$ &$[\bm{\tau}_{min},\bm{\tau}_{max}]$  &  -$\bm{a}_b$ \\
Stiffness feasibility  &  \ref{eq:pol_sfr_rob}& $\Delta\bm{x}$ & $J^T$ & - & $\bm{\tau}$ &$[\bm{\tau}_{min},\bm{\tau}_{max}]$ &  -$\bm{\tau}_b$  \\
\hline
 \multicolumn{8}{c}{Human musculoskeletal models} \\
\hline
Velocity &\ref{eq:velocity_polytope_human}  & $\dot{\bm{x}}$ & - & $J$ & $\dot{\bm{q}}$ & $\mathcal{P}_{\dot{\bm{q}}}$  & - \\
% Force/Wrench & $\bm{f}$ & $J^T$ & $N$ & $[\bm{F}_{min},\bm{F}_{max}]$ & $\bm{F}$ & $\bm{\tau}_b$ \\
Force/Wrench & \ref{eq:human_force_poly} &  $\bm{f}$ & $J^T$ & $N$  & $\bm{F}$ & $[\bm{F}_{min},\bm{F}_{max}]$ &  -$\bm{\tau}_b$ \\
Acceleration  & \ref{eq:poly_acceleration_hum} & $\ddot{\bm{x}}$ & - & $JM^{-1}N$  & $\bm{F}$ & $[\bm{F}_{min},\bm{F}_{max}]$ &  -$\bm{a}_b$ \\
% Stiffness feasibility  &$\Delta\bm{x}$ & $J^T$ & $N$ & $[\bm{F}_{min},\bm{F}_{max}]$ & $\bm{F}$ & $\bm{\tau}_b$ \\
Stiffness feasibility & \ref{eq:stiffness_human_all}  &$\Delta\bm{x}$ & $J^TK_c$ & $N$ & $\bm{F}$ & $[\bm{F}_{min},\bm{F}_{max}]$ &  -$\bm{\tau}_b$ \\
\hline
\end{tabular}
\caption{Comparison of Polytope-based Physical Ability Metrics}
\label{tab:unifying_metrics_human_robot}
\end{table}


Additionally, apart from being able to represent accurately both human's and robot's physical abilities, polytope algebra enables efficient operations over polytopes, such as sum and intersection, providing tools for characterising their common physical abilities when they are interacting physically to execute a task. As the resulting joint physical ability has a polytope form as well, the polytope metrics enable expressing human's, robot's, and collaborative physical abilities in a unified manner. In that way, they are providing opportunities to create more advanced collaboration scenarios, for example by being able to evaluate if different tasks are more suitable for robots, human operators or are they more suitable for their collaboration.

...
\todos{
\begin{itemize}
    \item polytopes for collaborative robot control
    \item human-centred robot control (but taking in consideration the human)
    \item capabilities of the dyad
    \item better than looking at the human as a disturbance (or predicting the human)
\end{itemize}
}
...
One traditional example of a collaborative task requiring physical interaction between multiple actors (humans and robots) is object carrying~\cite{Arai2000carrying,Kosuge1997carrying,Tsumugiwa2002carrying}. 

\todos{
\begin{itemize}
    \item To demonstrate this we show an example application of collaborative carrying
    \item Where multiple actors (robots and humans) collaborate physically in order to compensate the weight of the object (move it in space)
    \item each one of the actors has a certain capacity to carry the objects
    \item their common capacity is the some of the two
\end{itemize}
}

\subsection{Carrying capacity calculation}
...
\todos{
\begin{itemize}
    \item Carrying capacity can be calculated for robots and humans as their wrench capacity

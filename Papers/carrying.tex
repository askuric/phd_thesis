...
In the context of this experiment, the real-time carrying capacity calculation is implemented using the Python open-source library \textsc{Pycapacity}, developed in the context of this thesis and described more in detail in \Cref{ch:software}.
...
A straightforward control strategy, requiring each robot to compensate for half of object mass $f_{r_1}=f_{r_1}=0.5G$ is taken as baseline to evaluate the performance of the new method. This experiment is illustrated in the accompanying video\footnote{https://youtu.be/hApIv1oFuhk}.
...
As in the precious experiment, the real-time carrying capacity calculation, using VEPOLI$^2$ and $ICHM$ algorithms, is implemented using the Python open-source library \textsc{Pycapacity}. This library is developed in the context of this thesis and described more in detail in \Cref{ch:software}.
...
The operator's arm configuration is inferred in real-time using an OptiTrack~\cite{optitrack} motion capture system. The musculoskeletal model analysis is developed using the efficient biomechanics library \textsc{biorbd} \cite{Michaud2021}. 
...
% \subsubsection{Parallel with the admittance control}

% The proposed collaborative control strategy consists in modulating the weight the operator operators carries with respect to its changing capacity. From the operator's point of view, his task then becomes weight compensation of the object with constantly changing mass  
% \begin{equation}
% m = \frac{f_r}{g} =  \frac{G-f_r}{g}
% \end{equation}

% As the task of weight compensation consists in 
...
Moreover, in addition to to making sure that human's physical ability is not surpassed during the task executions, to further support \Myref{hyp:safety}, an additional task was incorporated into the collaborative robot control strategy aiming to improve the operator's safety and his engagement in the task. This task consisted in maintaining a constant relative load for the operator throughout the experiment, corresponding to 30\% of its carrying capacity. This concept was inspired by the Assist-As-Needed strategy as described in \citet{carmichael2013admittance}, commonly used in rehabilitation and assistance robotics. 

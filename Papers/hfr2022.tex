...
In order to avoid enumerating all the vertices of the high-dimensional joint torque polytope $\mathcal{P}_\tau$, as the most complex operation of this polytope transformation, Iterative Convex-Hull Method (ICHM) is used, allowing to directly enumerate low-dimensional \gls{cs} polytope $\mathcal{P}_x$. ICHM method is described in detail in \Cref{ch:algorihtm_ichm}. ICHM is an iterative method, based on successively applying \glspl{lp} and Convex-Hull algorithms and it is able to find both vertex and half-space representation of this polytope at the same time. ICHM algorithm is defined for a generic family of sets
\begin{equation}
...
\begin{table}[h!]
    ...
    \begin{tabular}{|c|ccccccc|}
        ...
        \hline
    \end{tabular}
    \caption{Franka Emika Panda robot \gls{js} actuator limits\footref{note:franka}.}
...
\begin{table}[h]
    ...
    \begin{tabular}{|c|cc|}
        ...
        \hline
    \end{tabular}
    \caption{Franka Emika Panda robot \gls{cs} kinematic limits\footref{note:franka}.}
...
A publicly available Python implementation of the proposed sampling-based reachable space approximation method can be found on GitLab\footnote{\url{https://GitLab.inria.fr/auctus-team/people/antunskuric/example/reachable_workspace}} as well as a short video\footnote{\url{https://www.youtube.com/watch?v=qHC4xMl_wdo}} demonstration of the method running in real-time. The implemented method uses the library \codet{Pinocchio}~\cite{pinocchio2021} for robot's forward kinematics while the non-convex Alpha-shape triangulation is implemented using \codet{CGAL}~\cite{cgal3Dalpha} library.
...
\todos{
Parts of this work have been published in the article \citet{Skuric2022hfr}.
}